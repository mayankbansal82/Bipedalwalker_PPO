...
class Env:
    ...
    def __init__(self,env_name,n_steps,gamma,gae_lambda):
        # self.env = gym.make(env_name,render_mode='human')
        self.env = gym.make(env_name)

        self.obs = self.env.reset()[0]
        # self.obs = self.obs.astype(np.float32)  # Convert self.obs to float32
        # print(self.obs[0])
    ...
    def steps(self,agent_policy, agent_value):
        ...
        for s in range(self.n_steps):
            ...
            # ag_mean = agent_policy(torch.tensor(self.obs[0], dtype=torch.float32))
            ag_mean = agent_policy(torch.tensor(self.obs, dtype=torch.float32))


            # get an action following the policy distribution
            logstd = agent_policy.logstd.data.cpu().numpy()
            action = ag_mean.data.cpu().numpy() + np.exp(logstd) * np.random.normal(size=logstd.shape[0])
            #action = np.random.normal(loc=ag_mean.data.cpu().numpy(), scale=torch.sqrt(ag_var).data.cpu().numpy())
            action = np.clip(action, -1, 1)
            # action = action[0]
            # print (action)

            # state_value = float(agent_value(torch.tensor(self.obs[0],dtype=torch.float32)))
            state_value = float(agent_value(torch.tensor(self.obs,dtype=torch.float32)))
...
load_model = False
checkpoint_name = "checkpoints/checkpoint_PPO_BipedalWalker-v3_23_3.59.40_0.0004_0.001_2049_64.pth.tar"
...
if __name__ == '__main__':
    ...
    while n_iter < MAX_ITER:
        ...
        writer.add_scalar('100rew', np.mean(env.last_games_rews[-100:]), n_iter)
